// sorry for the mess with many sections commented out this is just a proof of concept
...
console.log(msg);
var localIP = '192.168.34.123';  //this can be gotten from Homey's mDNS probe
var username = `email@example.com`;  //this is the e-mail for the Enphase Enlighten account
var password = 'supersecretpassword';  //this is the password for the Enphase Enlighten account
...
var intervalProduction = 30; // Inverter count and watts updated every 30 seconds (1*60);
    ...
    if (inverterIndex>-1){  // only update when inverter serial number exists already
        if (invertersArray[inverterIndex].lastReportDate!=inverterData_lastReportDate){ // only update when reading is updated
            ...
            console.log("Inverter: "+JSON.stringify(invertersArray[inverterIndex].serialNumber)+
                        ...
                        " Watts: "+invertersArray[inverterIndex].lastReportWatts);
        }
...
if (JSON.stringify(softwareversion).includes("D7.")) {
    ...
    for (let loopindex = 0; loopindex<1000000; loopindex++){
